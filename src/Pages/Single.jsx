import React from 'react'
import editImg from "../img/edit.png"
import deleteImg from "../img/delete.png"
import "../Style.scss"
import Menu from './Menu'
export default function Single() {
  return (
    <div className='container-fluid'>
      <div className="row single">
        <div className="col-8 content">
          <img className='postImage' src="https://images.pexels.com/photos/6489663/pexels-photo-6489663.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2" alt="postimage" />
          <div className="user">
            <img className='userImage' src="data:image/png;base64," alt="userimage" />
            <div className="info">
            Created By : <strong>Hamza</strong> &nbsp;
            <span className='text-muted'>2Days Ago</span>
            </div>
            <div className="edit">
              <img src={editImg} alt="edit Icon" />
            </div>
            <div className="delete">
              <img src={deleteImg} alt="edit Icon" />
            </div><br/>
          </div>
            <div>
              <center>
            <h1>This is single post </h1>
            <p>Lorem ipsum dolor Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores quas, nemo dicta, aspernatur corporis cupiditate magni non voluptates, sint itaque rem at aut consectetur corrupti voluptatibus velit fuga natus labore molestiae debitis temporibus voluptatum sunt nisi enim! Sit est asperiores, provident id fugiat cupiditate at nihil quos atque minus eveniet reprehenderit sequi corporis, officiis quia, necessitatibus pariatur tempora ab quisquam natus corrupti. Repellendus velit fuga alias! Quam sunt reiciendis ut eligendi recusandae, praesentium error dolorum voluptates provident </p><p>ipsam tempora, dolorem consequuntur assumenda quasi dignissimos delectus illum, aliquam sint quos nobis optio? Eum illo eligendi nisi quasi enim exercitationem, numquam odio! sit amet consectetur adipisicing elit. Nostrum maiores excepturi placeat recusandae harum quia, soluta nobis eius facere libero.</p>

              </center>
            </div>
        </div>
      <div className='col-md-4'>
        <Menu/>
      </div>
      </div>
    </div>
  )
}
